...
resource "aws_route53_record" "ingress_external" {
  zone_id = "${data.aws_route53_zone.public.zone_id}"
...
// NOTE: There is probably a better way.  We need
// an internal *.apps.cluster_domain as well pointing
// to the external LB since I did not create an internal
// LB I think...maybe relook at this again.

resource "aws_route53_record" "ingress_internal" {
