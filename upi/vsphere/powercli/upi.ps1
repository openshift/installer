...
$installRpSettings = Get-UPIInstallResourcePoolSettings -Metadata $metadata
$installResourcePoolName = $installRpSettings.PoolName
$manageInstallResourcePool = $installRpSettings.ManagePool
...
foreach ($fd in $fds)
{
    ...
    # Resource pool: create per infraID (default) or use existing pool when $vsphere_install_resource_pool_name is set (e.g. CI ipi-ci-clusters)
    ...
    $rp = Get-ResourcePool -Server $viserver -Name $installResourcePoolName -Location $clusterObj -ErrorAction continue 2>$null

    if ($manageInstallResourcePool) {
        if (-Not $?) {
            Write-Output "Creating resource pool $($installResourcePoolName) in datacenter $($fd.datacenter)"
            $rp = New-ResourcePool -Server $viserver -Name $installResourcePoolName -Location $clusterObj
            New-TagAssignment -Server $viserver -Entity $rp -Tag $tag > $null
        }
    } elseif (-Not $?) {
        throw "Existing resource pool '$($installResourcePoolName)' not found under cluster $($fd.cluster) on $($fd.server)"
    }
...
$rp = Get-ResourcePool -Name $installResourcePoolName -Location $(Get-Cluster -Server $fds[0].server -Name $($fds[0].cluster))
