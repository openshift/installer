	...
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err != nil {
				...
				err2 := cluster.API.OpenShift.LogClusterOperatorConditions()
				if err2 != nil {
					logrus.Error(fmt.Sprintf("Attempted to gather ClusterOperator status after wait failure: %s. Agent-based installer succeeded with the cluster installation but the error is caused due to Cluster Operator's status availability conditions.", err2))
				}
