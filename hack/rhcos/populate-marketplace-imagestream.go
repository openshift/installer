...
func (s marketplaceStream) azure(ctx context.Context, arch string) error {
	...
	if s[arch].Azure.OCP, err = azClient.OCP(ctx, arch, rel); err != nil {
		return err
	}

	if s[arch].Azure.OPP, err = azClient.OCP(ctx, arch, rel); err != nil {
		return err
	}

	if s[arch].Azure.OKE, err = azClient.OKE(ctx, arch, rel); err != nil {
		return err
	}
