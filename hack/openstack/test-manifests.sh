...
print_help() {
	...
	echo -e "\\t-b\\t\\tbootstrap flavor (used as BOOTSTRAP_FLAVOR in install-config templates)"
...
fill_install_config() {
	...
	sed '
		...
		s|${\?BOOTSTRAP_FLAVOR}\?|'"${bootstrap_flavor}"'|;
		s|${\?API_FIP}\?|'"${api_fip}"'|;
...
while getopts a:b:c:e:f:i:t:pr:h o; do
	case "$o" in
		a) api_fip="$OPTARG" ;;
		b) bootstrap_flavor="$OPTARG" ;;
		c) os_cloud="$OPTARG" ;;
