...
resource "google_dns_record_set" "etcd_srv_discover" {
  ...
  name         = "${var.tls_enabled ? "_etcd-server-ssl._tcp.${var.cluster_name}.${var.base_domain}" : "_etcd-server._tcp.${var.cluster_name}.${var.base_domain}"}."
  type         = "SRV"
  managed_zone = "${var.managed_zone_name}"
...
resource "google_dns_record_set" "etcd_srv_client" {
  count        = "${var.etcd_dns_enabled ? 1 : 0}"
  name         = "${var.tls_enabled ? "_etcd-client-ssl._tcp.${var.cluster_name}.${var.base_domain}" : "_etcd-client._tcp.${var.cluster_name}.${var.base_domain}"}."
  type         = "SRV"
  managed_zone = "${var.managed_zone_name}"
