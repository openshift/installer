...
resource "local_file" "etcd_ca_crt" {
  count    = "${var.self_signed || var.etcd_ca_cert_path != "/dev/null" ? 1 : 0}"
...
resource "local_file" "etcd_client_crt" {
  count    = "${var.self_signed || var.etcd_client_cert_path != "/dev/null" ? 1 : 0}"
...
resource "local_file" "etcd_client_key" {
  count    = "${var.self_signed || var.etcd_client_key_path != "/dev/null" ? 1 : 0}"
