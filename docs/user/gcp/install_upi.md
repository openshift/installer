...
export MASTER_DISK_SIZE="128GB"
export WORKER_DISK_SIZE="128GB"
export MASTER_DISK_TYPE="pd-ssd"
export WORKER_DISK_TYPE="pd-ssd"
export MASTER_MACHINE_TYPE="n1-standard-4"
export WORKER_MACHINE_TYPE="n1-standard-4"

export deprovision_commands_file="deprovision.sh"
export bootstrap_deprovision_commands_file="bootstrap_destroy.sh"
```

**Note**: If you use the INFRA_ID from the metadata.json file, then you can execute `openshift-install destroy cluster --dir=.` option to cleanup the resources
...
eval "gcloud compute instances create ${INFRA_ID}-bootstrap --boot-disk-size=${MASTER_DISK_SIZE} --image=${CLUSTER_IMAGE} --metadata=^#^user-data='{\"ignition\":{\"config\":{\"replace\":{\"source\":\"${BOOTSTRAP_IGN}\"}},\"version\":\"3.2.0\"}}' --machine-type=${MASTER_MACHINE_TYPE} --zone=${ZONE_0} --tags=${INFRA_ID}-master,${INFRA_ID}-bootstrap --subnet=${CONTROL_SUBNET} --address=${public_ip}"
sleep 3
...
cat << EOF >> "${bootstrap_deprovision_commands_file}"
...
- `bootstrap_deprovision_commands_file`: the filename (including the path) where the bootstrap deprovision commands will be added
...
ZONES_ARRAY=($ZONES)
for zone in "${ZONES_ARRAY[@]}"; do
    gcloud compute instances create ${INFRA_ID}-${machine_role}-${index} --boot-disk-size=${MASTER_DISK_SIZE} --boot-disk-type=${MASTER_DISK_TYPE} --image=${CLUSTER_IMAGE} --metadata=^#^user-data="${MASTER_IGNITION}" --machine-type=${MASTER_MACHINE_TYPE} --zone=${zone} --no-address --service-account=${MASTER_SERVICE_ACCOUNT} --scopes=https://www.googleapis.com/auth/cloud-platform --tags=${INFRA_ID}-${machine_role} --subnet=${CONTROL_SUBNET}
    sleep 3
...
- `CONTROL_SUBNET`: the URI to the control subnet
- `CLUSTER_IMAGE`: the URI to the RHCOS image
- `MASTER_MACHINE_TYPE`: the machine type of the instance (for example n1-standard-4)
- `MASTER_DISK_SIZE`: The machine boot disk size (in GB).
- `MASTER_DISK_TYPE`: The type of boot disk (for example pd-ssd).
- `MASTER_SERVICE_ACCOUNT`: the email address for the master service account created above
- `MASTER_IGNITION`: the contents of the master.ign file
...
if [[ ${BASH_VERSION%%.*} -lt 4 ]]; then
  ...
  worker_instance_zones=`gcloud compute machine-types list --filter="zone:(${REGION}) AND name=(${WORKER_MACHINE_TYPE})" --format=json | jq -r '.[].zone'`
...
for zone in "${WORKER_ZONES[@]}"; do
    gcloud compute instances create ${INFRA_ID}-${machine_role}-${index} --boot-disk-size=${WORKER_DISK_SIZE}GB --boot-disk-type=${WORKER_DISK_TYPE} --image=${CLUSTER_IMAGE} --metadata=^#^user-data="${WORKER_IGNITION}" --machine-type=${WORKER_MACHINE_TYPE} --zone=${zone} --no-address --service-account=${WORKER_SERVICE_ACCOUNT} --scopes=https://www.googleapis.com/auth/cloud-platform --tags=${INFRA_ID}-${machine_role} --subnet=${COMPUTE_SUBNET}
    sleep 3
...
- `CLUSTER_IMAGE`: the URI to the RHCOS image
- `WORKER_MACHINE_TYPE`: the machine type of the instance (for example n1-standard-4)
- `WORKER_DISK_SIZE`: The machine boot disk size (in GB).
- `WORKER_DISK_TYPE`: The type of boot disk (for example pd-ssd).
- `WORKER_SERVICE_ACCOUNT`: the email address for the worker service account created above
- `WORKER_IGNITION`: the contents of the worker.ign file
...
Execute the `$bootstrap_deprovision_commands_file` script. This script should be populated with the commands to
destroy the remaining bootstrap resources; i.e. the instance, address, etc. 
