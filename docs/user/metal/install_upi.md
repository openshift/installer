...
7. Watching your installation (production control-plane, cluster available)
...
The administrators can use the `wait-for control-plane-ready` target of the OpenShift Installer to monitor cluster bootstrapping. The command succeeds when it notices `bootstrap-complete` event from Kubernetes APIServer. This event is generated by the bootstrap machine after the Kubernetes APIServer has been bootstrapped on the control plane machines. For example,

```console
$ openshift-install --dir test-bare-metal wait-for control-plane-ready
...
After [waiting for the production control-plane][#monitor-for-the-production-control-plane], the bootstrap machine can be removed from the API pool with:
