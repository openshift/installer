...
In order to SSH into the master nodes as user core, it is necessary to include an administrator's SSH key during the installation. This key can be defined in the `install-config.yml` under the variable `sshKey`. The install-config.yml can be generated by running 
```sh 
