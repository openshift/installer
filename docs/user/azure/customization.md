...
If `resourceGroupName` is specified, it must differ from `networkResourceGroupName` if both are provided. This is because the cluster resource group is deleted on cluster destroy, and existing network resources must not be accidentally removed.
