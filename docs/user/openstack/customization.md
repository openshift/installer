...
* `serverGroupPolicy` (optional string): Server group policy to apply to the group that will contain the machines in the pool. Defaults to "soft-anti-affinity". Note that a strict "affinity" policy prevents migrations, and therefore affects OpenStack upgrades. Allowed values are "affinity", "soft-affinity", "anti-affinity", "soft-anti-affinity". It is not possible to change a server group policy or a server's affiliation to a group after creation.
