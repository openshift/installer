...
func FetchCoreOSBuild(ctx context.Context) (*stream.Stream, error) {
	...
	mktBody, err := fetchRawMarketplaceStream(ctx)
	if err != nil {
...
func fetchRawMarketplaceStream(ctx context.Context) ([]byte, error) {
	file, err := data.Assets.Open(getMarketplaceStreamFileName())
	if err != nil {
		return nil, fmt.Errorf("failed to read embedded marketplace stream metadata: %w", err)
	}
	defer file.Close()

	body, err := io.ReadAll(file)
	if err != nil {
		return nil, fmt.Errorf("failed to read marketplace stream metadata: %w", err)
	}
	return body, nil
}
