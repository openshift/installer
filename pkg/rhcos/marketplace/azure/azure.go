...
func (az *MarketplaceStream) Populate(ctx context.Context, arch, rel string) (*rhcos.AzureMarketplace, error) {
	...
	if s.OCP, err = az.ocp(ctx, pubRH, arch, rel); err != nil {
		return nil, err
	}

	if s.OPP, err = az.opp(ctx, pubRH, arch, rel); err != nil {
		return nil, err
	}

	if s.OKE, err = az.oke(ctx, pubRH, arch, rel); err != nil {
		return nil, err
	}

	if s.OCPEMEA, err = az.ocp(ctx, pubEMEA, arch, rel); err != nil {
		return nil, err
	}

	if s.OPPEMEA, err = az.opp(ctx, pubEMEA, arch, rel); err != nil {
		return nil, err
	}

	if s.OKEEMEA, err = az.oke(ctx, pubEMEA, arch, rel); err != nil {
		return nil, err
	}
...
func (az *MarketplaceStream) ocp(ctx context.Context, pub, arch, release string) (*rhcos.AzureMarketplaceImages, error) {
	if arch != x86 {
		return nil, nil
	}
	return az.getImages(ctx, paidImageQuery(pub, release, offerOCP), arch)
}

func (az *MarketplaceStream) opp(ctx context.Context, pub, arch, release string) (*rhcos.AzureMarketplaceImages, error) {
	if arch != x86 {
		return nil, nil
	}
	return az.getImages(ctx, paidImageQuery(pub, release, offerOPP), arch)
}

func (az *MarketplaceStream) oke(ctx context.Context, pub, arch, release string) (*rhcos.AzureMarketplaceImages, error) {
	if arch != x86 {
		return nil, nil
	}
	return az.getImages(ctx, paidImageQuery(pub, release, offerOKE), arch)
}
