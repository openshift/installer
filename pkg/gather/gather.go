...
func CombineArchives(archiveName string, archives map[string]string) error {
	...
	for archive, subDirectory := range archives {
		...
		directory := filepath.Base(strings.TrimSuffix(archive, suffix)) + "/"
		...
		for true {
			...
			newHeaderName := filepath.Join(combinedDirectory, strings.Replace(header.Name, directory, subDirectory, 1))
