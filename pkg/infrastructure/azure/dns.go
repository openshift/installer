...
func createDNSEntries(ctx context.Context, in *createDNSEntriesInput) error {
	...
	if len(in.privateIPv4Addresses) > 0 {
		for _, ipv4Address := range in.privateIPv4Addresses {
	...
	if len(in.privateIPv6Addresses) > 0 {
		for _, ipv6Address := range in.privateIPv6Addresses {
