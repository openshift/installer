...
func getVPCFromSubnets(ctx context.Context, ic *installconfig.InstallConfig, subnetIDs []string) (string, error) {
	...
	if vpcID == "" {
		return "", fmt.Errorf("no VPC found in subnets %v", subnetIDs)
	}
...
func removeS3Bucket(ctx context.Context, region string, bucketName string, endpoints []awstypes.ServiceEndpoint) error {
	...
	// TODO: what should we set the key to here?
	// TODO: is this created in CAPA
	//if _, err := client.DeleteObject(ctx, &s3.DeleteObjectInput{Bucket: aws.String(bucketName)}); err != nil {
	//	var noSuchBucketError *s3types.NoSuchBucket
	//	if !errors.As(err, &noSuchBucketError) {
	//		logrus.Warnf("failed to delete S3 object for bucket %s: %v", bucketName, err)
	//		return err
	//	}
	//}
	...
	if _, err := client.DeleteBucket(ctx, &s3.DeleteBucketInput{Bucket: aws.String(bucketName)}); err != nil {
		...
		if errors.As(err, &apiErr) && strings.Contains(apiErr.ErrorCode(), "NoSuchBucket") {
