...
func newIdleTimeoutReader(r io.Reader, idle time.Duration, cancel context.CancelFunc) *idleTimeoutReader {
...
func downloadRHCOSImage(imageURL string, sha256Checksum string) (string, error) {
	...
	err = retry.DoFunc(downloadRetryCount, downloadRetryDelay, func() error {
		...
		hasher := sha256.New()
		// Wrap with an idle timeout — cancel if no data for 5 min
		reader := newIdleTimeoutReader(resp.Body, 5*time.Minute, cancel)
