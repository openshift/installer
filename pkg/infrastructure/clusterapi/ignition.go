...
func EditIgnition(in IgnitionInput, platform string, publicIPAddresses, privateIPAddresses []string) (*IgnitionOutput, error) {
	...
	editedMasterIgn, err := updatePointerIgnition(in, privateIPAddresses, "master")
	if err != nil {
		return nil, fmt.Errorf("failed to update Master Pointer Ignition: %w", err)
	}
	logrus.Debugf("Successfully updated master pointer ignition with API LB IP")
	editedWorkerIgn, err := updatePointerIgnition(in, privateIPAddresses, "worker")
	if err != nil {
		return nil, fmt.Errorf("failed to update Worker Pointer Ignition: %w", err)
	}
	logrus.Debugf("Successfully updated worker pointer ignition with API LB IP")
...
func updatePointerIgnition(in IgnitionInput, privateLBs []string, role string) ([]byte, error) {
	...
	case "master":
		...
		if len(privateLBs) > 0 {
			...
			editedIgnBytes, err := json.Marshal(ignData)
			if err != nil {
				return nil, fmt.Errorf("failed to convert master pointer ignition data to json: %w", err)
			}
	...
	case "worker":
		...
		if err != nil {
			return nil, fmt.Errorf("failed to unmarshal worker ignition: %w", err)
		}
		if len(privateLBs) > 0 {
			ignitionHost := net.JoinHostPort(privateLBs[0], "22623")
			ignData.Ignition.Config.Merge[0].Source = ignutil.StrToPtr(func() *url.URL {
			...
			editedIgnBytes, err := json.Marshal(ignData)
			if err != nil {
				return nil, fmt.Errorf("failed to convert worker pointer ignition data to json: %w", err)
			}
			return editedIgnBytes, nil
		}
