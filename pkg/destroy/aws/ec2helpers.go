...
func (o *ClusterUninstaller) deleteEC2(ctx context.Context, client *ec2v2.Client, arn arn.ARN, logger logrus.FieldLogger) error {
...
func deleteEC2SubnetsByVPC(ctx context.Context, client *ec2v2.Client, vpc string, failFast bool, logger logrus.FieldLogger) error {
	...
	for paginator.HasMorePages() {
		...
		for _, subnet := range page.Subnets {
			...
			if err != nil {
				err = fmt.Errorf("deleting EC2 subnet %s: %w", *subnet.SubnetId, err)
				if lastError != nil {
					logger.Debug(lastError)
				}
				lastError = err
