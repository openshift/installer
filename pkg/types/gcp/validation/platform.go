...
func ValidatePlatform(p *gcp.Platform, fldPath *field.Path, ic *types.InstallConfig) field.ErrorList {
	...
	if p.PrivateZone != nil {
		if p.NetworkProjectID == "" {
			allErrs = append(allErrs, field.Required(fldPath.Child("privateZone"), "must provide a network project id when a private dns zone is specified"))
		}
		if p.PrivateZone.Zone == "" {
			allErrs = append(allErrs, field.Required(fldPath.Child("privateZone").Child("zone"), "must provide a zone id when a private dns zone is specified"))
		}
	}
