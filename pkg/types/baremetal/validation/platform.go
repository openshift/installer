...
func ValidatePlatform(p *baremetal.Platform, agentBasedInstallation bool, n *types.Networking, fldPath *field.Path, c *types.InstallConfig) field.ErrorList {
	...
	enabledCaps := GetEnabledCapabilities(c.Capabilities)
...
func GetEnabledCapabilities(capabilities *types.Capabilities) sets.Set[configv1.ClusterVersionCapability] {
	...
	if capabilities != nil {
		if capabilities.BaselineCapabilitySet != "" {
			baseSet := configv1.ClusterVersionCapabilitySets[capabilities.BaselineCapabilitySet]
			for _, cap := range baseSet {
				enabledCaps.Insert(cap)
			}
		}
		if capabilities.AdditionalEnabledCapabilities != nil {
			for _, cap := range capabilities.AdditionalEnabledCapabilities {
				enabledCaps.Insert(cap)
			}
		}
	}
