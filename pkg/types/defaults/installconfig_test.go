...
func TestSetInstallConfigDefaults(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NotEmpty(t, tc.config.ClusterID, "expected non-empty cluster ID")
			if tc.expected.ClusterID == "" {
				// Default is a new UUID for ClusterID. Set the expected value to
				// the generated value so that the subsequent equals test will pass.
				tc.expected.ClusterID = tc.config.ClusterID
			}
