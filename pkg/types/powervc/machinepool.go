...
func (o *MachinePool) Set(required *MachinePool) {
	...
	if required.AdditionalNetworkIDs != nil {
		// Avoid this golint error:
		// appendAssign: append result not assigned to the same slice (gocritic)
		ani := required.AdditionalNetworkIDs[:0:0]
		ani = append(ani, required.AdditionalNetworkIDs...)
		o.AdditionalNetworkIDs = ani
	}

	if required.AdditionalSecurityGroupIDs != nil {
		// Avoid this golint error:
		// appendAssign: append result not assigned to the same slice (gocritic)
		asgi := required.AdditionalSecurityGroupIDs[:0:0]
		asgi = append(asgi, required.AdditionalSecurityGroupIDs...)
		o.AdditionalSecurityGroupIDs = asgi
	}
