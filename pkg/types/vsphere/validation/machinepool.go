...
func ValidateMachinePool(platform *vsphere.Platform, machinePool *types.MachinePool, fldPath *field.Path) field.ErrorList {
	...
	// Either number set by user or a default value
	if numCPUs <= 0 {
	...
	if numCoresPerSocket > numCPUs {
		...
		errorMsg := fmt.Sprintf("numCPUs specified should be a multiple of cores per socket (which is by default %d)", defaultCoresPerSocket)
		allErrs = append(allErrs, field.Invalid(fldPath.Child("cpus"), numCPUs, errorMsg))
	}
