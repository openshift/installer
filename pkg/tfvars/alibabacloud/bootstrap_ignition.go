...
// Ref: pkg/tfvars/aws/bootstrap_ignition.go
...
func generateIgnitionShim(bootstrapConfigURL string, userCA string) (string, error) {
	...
	if len(data) > 16000 {
		return "", fmt.Errorf("rendered bootstrap ignition shim exceeds the 16KB limit for Alibaba Cloud user data -- try reducing the size of your CA cert bundle")
	}
