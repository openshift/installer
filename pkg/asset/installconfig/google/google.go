...
import (
	...
	"github.com/openshift/installer/pkg/types/google/validation"
)

// Platform collects GCP-specific configuration.
func Platform() (*google.Platform, error) {
	...
	if !ok {
		panic(fmt.Sprintf("installer bug: invalid default GCP region %q", defaultRegion))
	}
	...
	// 	} else {
	// 		logrus.Warnf("Unrecognized GCP region %q, defaulting to %s", *defaultRegionPointer, defaultRegion)
	// 	}
	// }
...
func getCredentials() error {
	...
	tmpl, err := template.New("google-credentials").Parse(`# Created by openshift-install
...
`)
	...
	logrus.Infof("Writing GCP credentials to %q (https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html)", path)
