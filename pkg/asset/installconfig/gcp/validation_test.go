...
func TestValidateInstanceType(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			expectedError:       true,
			expectedErrMsg:      `^\[instance.confidentialCompute: Invalid value: "Enabled": Unknown confidential computing technology Enabled\]`,
		},
		{
			name:                "AMDEncryptedVirtualization confidential compute with correct on host maintenance",
			zones:               []string{"a"},
			instanceType:        "c2d-standard-4",
			diskType:            "pd-ssd",
			onHostMaintenance:   "Terminate",
			...
			expectedError:       true,
			expectedErrMsg:      `^\[instance.confidentialCompute: Invalid value: "AMDEncryptedVirtualization": Unknown confidential computing technology AMDEncryptedVirtualization\]`,
		},
		{
