...
func validateSubnets(ctx context.Context, meta *Metadata, fldPath *field.Path, providedSubnets []awstypes.Subnet, networking *types.Networking, publish types.PublishingStrategy, clusterName string) field.ErrorList {
	...
	if publish == types.InternalPublishingStrategy && len(subnetDataGroups.Public) > 0 {
		// The presence of user-provided public subnets in a private cluster installation might pose a security risk.
		// However, we provision bootstrap nodes in public subnets if any is available. Thus, such a case is still valid.
...
// Untagged subnets may be selected by the CCM, leading to various bugs, RFEs, and support cases.
