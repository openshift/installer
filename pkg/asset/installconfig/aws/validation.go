...
func validateServiceEndpoints(fldPath *field.Path, region string, services []awstypes.ServiceEndpoint) field.ErrorList {
	...
	for id, service := range services {
		if !sets.New(requiredServices...).Has(service.Name) {
		...
		endpointOverrides[service.Name] = service.URL
	}

	var errs []error
	// validate the service endpoint for each of the required services. This will be the default endpoint for
	// services where the user did not override the service endpoint through the install-config.
	for _, service := range requiredServices {
		if urlOverride, ok := endpointOverrides[service]; ok {
			if err := validateEndpointAccessibility(urlOverride); err != nil {
				logrus.Debugf("failed to access %s endpoint at %s", service, urlOverride)
				errs = append(errs, field.Invalid(fldPath, region, fmt.Sprintf("failed to access %s endpoint override %s", service, urlOverride)))
			}
		} else {
			defaultEndpoint, err := getDefaultServiceEndpoint(region, service, endpointOptions{
				DisableHTTPS:         false,
				UseDualStackEndpoint: awsv2.DualStackEndpointStateDisabled,
			})
			if err != nil {
				errs = append(errs, field.Invalid(fldPath, region, fmt.Sprintf("could not determine %s endpoint for region %s", service, region)))
			} else {
				if err := validateEndpointAccessibility(defaultEndpoint.URL); err != nil {
					logrus.Debugf("failed to access %s endpoint at %s", service, defaultEndpoint.URL)
					errs = append(errs, field.Invalid(fldPath, region, fmt.Sprintf("failed to access default %s endpoint", service)))
				}
			}
		}
	}
