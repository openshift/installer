...
func TestValidateForProvisioning(t *testing.T) {
	cases := []struct {
		...
		icOptions: []icOption{icBuild.withBaseBYO(), icBuild.withPublish(types.InternalPublishingStrategy), icBuild.withHostedZone(validHostedZoneName)}, // HERE
	}, {
		...
		icOptions: []icOption{icBuild.withBaseBYO(), icBuild.withHostedZone(validHostedZoneName)},
	}, {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			err := ValidateForProvisioning(t.Context(), route53Client, ic, meta)
			if test.expectedErr == "" {
				assert.NoError(t, err)
			} else {
				if assert.Error(t, err) {
...
func validInstanceTypes() map[string]InstanceType {
	return map[string]InstanceType{
		"t2.small": {
			DefaultVCpus: 1,
			MemInMiB:     2048,
			Arches:       []ec2types.ArchitectureType{ec2types.ArchitectureTypeX8664},
		},
		"m5.large": {
			DefaultVCpus: 2,
			MemInMiB:     8192,
		...
		"m6g.xlarge": {
			DefaultVCpus: 4,
			MemInMiB:     16384,
			Arches:       []ec2types.ArchitectureType{ec2types.ArchitectureTypeArm64},
		},
	}
}
