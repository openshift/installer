...
// Notice that existing instance profiles should already have a role attached.
func IncludesCreateInstanceRole(installConfig *types.InstallConfig) bool {
	...
	// If compute stanza is not defined, we know it'll inherit the value from DefaultMachinePlatform
	mpool := installConfig.AWS.DefaultMachinePlatform
	return mpool == nil || (len(mpool.IAMRole) == 0 && len(mpool.IAMProfile) == 0)
}
...
// IncludesCreateInstanceProfile checks if at least one instance profile will be created the the installer.
