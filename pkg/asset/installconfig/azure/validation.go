...
func validateNetworks(client API, p *aztypes.Platform, machineNetworks []types.MachineNetworkEntry, fieldPath *field.Path) field.ErrorList {
	...
	if p.VirtualNetwork != "" {
		...
		computeSubnet, err := client.GetComputeSubnet(context.TODO(), p.NetworkResourceGroupName, p.VirtualNetwork, computeSubnetName)
		if err != nil {
			return append(allErrs, field.Invalid(fieldPath.Child("computeSubnet"), computeSubnet, "failed to retrieve compute subnet"))
		}

		allErrs = append(allErrs, validateSubnet(client, fieldPath.Child("computeSubnet"), computeSubnet, computeSubnetName, machineNetworks)...)

		controlPlaneSubnet, err := client.GetControlPlaneSubnet(context.TODO(), p.NetworkResourceGroupName, p.VirtualNetwork, controlPlaneSubnetName)
		if err != nil {
			return append(allErrs, field.Invalid(fieldPath.Child("controlPlaneSubnet"), controlPlaneSubnetName, "failed to retrieve control plane subnet"))
		}

		allErrs = append(allErrs, validateSubnet(client, fieldPath.Child("controlPlaneSubnet"), controlPlaneSubnet, controlPlaneSubnetName, machineNetworks)...)
		if len(allErrs) == 0 {
			for index, subnet := range p.Subnets {
				computeSubnet, err := client.GetComputeSubnet(context.TODO(), p.NetworkResourceGroupName, p.VirtualNetwork, subnet.Name)
				if err != nil {
					return append(allErrs, field.Invalid(fieldPath.Child("computeSubnet"), computeSubnet, "failed to retrieve compute subnet"))
				}
				addressPrefix := []string{}
				if computeSubnet.AddressPrefix != nil {
					addressPrefix = append(addressPrefix, *computeSubnet.AddressPrefix)
				}
				if computeSubnet.AddressPrefixes != nil {
					addressPrefix = append(addressPrefix, *computeSubnet.AddressPrefixes...)
				}
				p.Subnets[index].SubnetCIDR = addressPrefix
			}
		}
	}
...
func validateCustomSubnetsAndNatGateways(client API, fldPath *field.Path, ic *types.InstallConfig) field.ErrorList {
	...
	if virtualNetwork == "" {
		...
		vnet, err := client.GetVirtualNetwork(context.TODO(), networkResourceGroupName, virtualNetwork)
		if err != nil {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("virtualNetwork"), virtualNetwork, "unable to get virtual network"))
		} else {
			vnetCIDR = []ipnet.IPNet{}
			for _, address := range *vnet.AddressSpace.AddressPrefixes {
	...
	for _, subnet := range subnetSpec {
		if len(subnet.SubnetCIDR) == 0 {
			...
			subnetCIDRList[subnet.Name] = subnet.SubnetCIDR
		}
	}
