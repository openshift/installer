...
func setSubnetsBYOVPC(in *networkInput) error {
	...
	if subnetIDsByRole := in.Subnets.GetSubnetIDsGroupedByRole(); len(subnetIDsByRole) > 0 {
		...
		// TODO: Remove this check when CAPA version is bumped with support for public only networking.
		// https://github.com/kubernetes-sigs/cluster-api-provider-aws/commit/f3398998b4000ce24dda824e4b6e7504dd1810ee
		// If we don't set the subnets for the internal LB, CAPA will try to use private subnets but there aren't any in
		// public-only mode.
		in.Cluster.Spec.ControlPlaneLoadBalancer.Subnets = in.Cluster.Spec.NetworkSpec.Subnets.IDs()
	}
