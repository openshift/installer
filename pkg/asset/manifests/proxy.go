...
// BuildNoProxySet expands the user's NoProxy configuration with basic entries required for the internal cluster communication to avoid the user's proxy. This function only adds universal etnries that are relevant in all platforms.
// The caller can add platform-specific entries as needed.
func BuildNoProxySet(installConfig *types.InstallConfig) sets.Set[string] {
	set := sets.New[string](
		"127.0.0.1",
		"localhost",
		".svc",
	...
	for _, network := range installConfig.Networking.ServiceNetwork {
		set.Insert(network.String())
	}

	for _, network := range installConfig.Networking.MachineNetwork {
		set.Insert(network.CIDR.String())
	}

	for _, network := range installConfig.Networking.ClusterNetwork {
		set.Insert(network.CIDR.String())
	}

	// Add internal API server hostname
	set.Insert("api-int." + installConfig.ClusterDomain())

	for _, userValue := range strings.Split(installConfig.Proxy.NoProxy, ",") {
		if userValue != "" {
