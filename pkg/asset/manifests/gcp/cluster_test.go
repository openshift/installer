	...
	tests := []struct {
	...
	}{
		{
			...
			validateRules: func(t *testing.T, rules []capg.FirewallRule, infraID string, machineCIDR string) {
				...
				// For external publishing, should have base + additional health check ranges
				assert.Equal(t, []string{"35.191.0.0/16", "130.211.0.0/22", "209.85.152.0/22", "209.85.204.0/22"}, healthRule.SourceRanges)
		...
		},
		{
			...
			validateRules: func(t *testing.T, rules []capg.FirewallRule, infraID string, machineCIDR string) {
				...
				// For internal publishing, health checks should only have base ranges
				healthRule, found := ruleMap[infraID+"-health-checks"]
				if !assert.True(t, found, "health-checks rule should exist") {
					return
				}
				assert.Equal(t, []string{
					"35.191.0.0/16",
					"130.211.0.0/22",
				}, healthRule.SourceRanges, "Internal publishing should only have base health check ranges")
