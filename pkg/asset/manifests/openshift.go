...
func (o *Openshift) Generate(ctx context.Context, dependencies asset.Parents) error {
	...
	case vspheretypes.Name:
		...
		for _, vCenter := range installConfig.Config.VSphere.VCenters {
			...
			vsphereCred := VSphereCredsSecretData{
				...
				Base64encodeUsername: base64.StdEncoding.EncodeToString([]byte(credentials.User)),
				Base64encodePassword: base64.StdEncoding.EncodeToString([]byte(credentials.Password)),
			}
	...
	case awstypes.Name, openstacktypes.Name, powervctypes.Name, vspheretypes.Name, azuretypes.Name, gcptypes.Name, ibmcloudtypes.Name, ovirttypes.Name:
		...
		} else if platform == vspheretypes.Name && installConfig.Config.VSphere.CredentialType == vspheretypes.CredentialTypeComponentScoped {
			secrets, err := manualVSphereCredentialSecrets(installConfig.Config.VSphere)
			if err != nil {
				return err
			}
			for filename, secret := range secrets {
				data, err := yaml.Marshal(secret)
				if err != nil {
					return errors.Wrapf(err, "failed to marshal %s", filename)
				}
				assetData[filename] = data
			}
		}
...
func manualVSphereCredentialSecrets(platform *vspheretypes.Platform) (map[string]*corev1.Secret, error) {
	secrets := map[string]*corev1.Secret{
		"99_vsphere-machine-api-credentials.yaml": {
			ObjectMeta: metav1.ObjectMeta{Name: "vsphere-cloud-credentials", Namespace: "openshift-machine-api"},
			Data:       map[string][]byte{},
		},
		"99_vsphere-csi-credentials.yaml": {
			ObjectMeta: metav1.ObjectMeta{Name: "vmware-vsphere-cloud-credentials", Namespace: "openshift-cluster-csi-drivers"},
			Data:       map[string][]byte{},
		},
		"99_vsphere-cloud-controller-credentials.yaml": {
			ObjectMeta: metav1.ObjectMeta{Name: "vsphere-cloud-credentials", Namespace: "openshift-cloud-controller-manager"},
			Data:       map[string][]byte{},
		},
		"99_vsphere-problem-detector-credentials.yaml": {
			ObjectMeta: metav1.ObjectMeta{Name: "vsphere-cloud-credentials", Namespace: "openshift-cluster-storage-operator"},
			Data:       map[string][]byte{},
		},
	}
	...
	for _, vcenter := range platform.VCenters {
		if vcenter.ComponentCredentials == nil {
			return nil, errors.Errorf("vcenter %s is missing componentCredentials", vcenter.Server)
		}
		credentials := map[string]vspheretypes.Credential{
			"99_vsphere-machine-api-credentials.yaml":      vcenter.ComponentCredentials.MachineManagement,
			"99_vsphere-csi-credentials.yaml":              vcenter.ComponentCredentials.Storage,
			"99_vsphere-cloud-controller-credentials.yaml": vcenter.ComponentCredentials.CloudControllerManager,
			"99_vsphere-problem-detector-credentials.yaml": vcenter.ComponentCredentials.VSphereProblemDetector,
		}
		for filename, credential := range credentials {
			if credential.User == "" || credential.Password == "" {
				return nil, errors.Errorf("vcenter %s has incomplete credentials for %s", vcenter.Server, filename)
			}
			secrets[filename].Data[vcenter.Server+".username"] = []byte(credential.User)
			secrets[filename].Data[vcenter.Server+".password"] = []byte(credential.Password)
		}
	}
