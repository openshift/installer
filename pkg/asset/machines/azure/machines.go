...
func Machines(clusterID string, config *types.InstallConfig, pool *types.MachinePool, osImage, role, userDataSecret string, capabilities map[string]string, useImageGallery bool, dataDisks []machineapi.DataDisk) ([]machineapi.Machine, *machinev1.ControlPlaneMachineSet, error) {
	...
	for idx := int64(0); idx < total; idx++ {
		...
		provider, err := provider(platform, mpool, osImage, userDataSecret, clusterID, role, &azIndex, capabilities, useImageGallery, dataDisks)
		if err != nil {
...
func provider(platform *azure.Platform, mpool *azure.MachinePool, _ string, userDataSecret string, clusterID string, role string, azIdx *int, capabilities map[string]string, useImageGallery bool, dataDisks []machineapi.DataDisk) (*machineapi.AzureMachineProviderSpec, error) {
	var az string
