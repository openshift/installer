...
func provider(platform *azure.Platform, mpool *azure.MachinePool, osImage string, userDataSecret string, clusterID string, role string, azIdx *int, capabilities map[string]string, useImageGallery bool) (*machineapi.AzureMachineProviderSpec, error) {
	...
	if platform.DefaultMachinePlatform != nil {
		bootDiagnostics = getBootDiagnosticObject(platform.DefaultMachinePlatform.BootDiagnostics, platform.CloudName, role)
	}
	tempDiagnostics := getBootDiagnosticObject(mpool.BootDiagnostics, platform.CloudName, role)
	if tempDiagnostics != nil {
		bootDiagnostics = tempDiagnostics
	}
	if bootDiagnostics != nil {
		if bootDiagnostics.Boot.StorageAccountType != machineapi.CustomerManagedAzureDiagnosticsStorage {
			bootDiagnostics.Boot.CustomerManaged = nil
		}
...
func getBootDiagnosticObject(diag *azure.BootDiagnostics, cloudName azure.CloudEnvironment, role string) (bootDiagnostics *machineapi.AzureDiagnostics) {
	if diag == nil {
		if role == "master" {
