...
func GenerateMachines(clusterID, resourceGroup, subscriptionID string, in *MachineInput) ([]*asset.RuntimeFile, error) {
	...
	if in.Platform.DefaultMachinePlatform != nil && in.Platform.DefaultMachinePlatform.BootDiagnostics != nil {
		...
		if saURI := bootDiagStorageURIBuilder(in.Platform.DefaultMachinePlatform.BootDiagnostics, in.Platform.CloudName); saURI != "" {
	...
	if mpool.BootDiagnostics != nil {
		...
		if saURI := bootDiagStorageURIBuilder(mpool.BootDiagnostics, in.Platform.CloudName); saURI != "" {
			controlPlaneDiag.Boot.UserManaged = &capz.UserManagedBootDiagnostics{
				StorageAccountURI: saURI,
			}
		}
	}
	if controlPlaneDiag.Boot.StorageAccountType != capz.UserManagedDiagnosticsStorage {
		controlPlaneDiag.Boot.UserManaged = nil
	}
...
func bootDiagStorageURIBuilder(diag *aztypes.BootDiagnostics, cloudType aztypes.CloudEnvironment) string {
