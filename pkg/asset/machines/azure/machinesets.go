...
func MachineSets(clusterID string, config *types.InstallConfig, pool *types.MachinePool, osImage, role, userDataSecret string, capabilities map[string]string, useImageGallery bool, isAro bool) ([]*clusterapi.MachineSet, error) {
	...
	for idx, az := range azs {
		...
		provider, err := provider(platform, mpool, osImage, userDataSecret, clusterID, role, &idx, capabilities, useImageGallery, isAro)
		if err != nil {
