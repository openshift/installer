...
const (
	...
	// fencingCredentialsSecretFileName is the format string for constructing the Secret
	// filenames for Two Node with Fencing clusters
	fencingCredsSecretFileName = "99_openshift-cluster-api_fencing-creds-secrets-%s.yaml"
...
var (
	...
	fencingCredsFilenamePattern        = fmt.Sprintf(fencingCredsSecretFileName, "*")
...
func (m *Master) Generate(ctx context.Context, dependencies asset.Parents) error {
	...
	if ic.Platform.Name() == powervstypes.Name {
		...
		// This is only used by Two Node with Fencing
		...
		if pool.Fencing != nil && pool.Fencing.Credentials != nil {
			credentials, err := gatherFencingCredentials(pool.Fencing.Credentials)
			if err != nil {
				return err
			}

			secrets, err := createSecretAssetFiles(credentials, fencingCredsSecretFileName)
			if err != nil {
				return errors.Wrap(err, "failed to gather fencing credentials for control plane hosts")
			}
			m.SecretFiles = append(m.SecretFiles, secrets...)
		}
	}
...
func gatherFencingCredentials(credentials []*types.Credential) ([]corev1.Secret, error) {
	...
	for _, credential := range credentials {
		if credential.HostName == "" || credential.Username == "" || credential.Password == "" || credential.Address == "" {
			return _, fmt.Errorf("Incomplete fencing credentials for host with HostName='%s'", credential.HostName)
		}
		...
		secret := &corev1.Secret{
			...
			ObjectMeta: metav1.ObjectMeta{
				Name:      fmt.Sprintf("%s-fencing-cred-secret", credential.HostName),
