...
func (i *RegistriesConf) Load(f asset.FileFetcher) (bool, error) {
	...
	if err = i.finish("Loaded"); err != nil {
		return false, err
	}
...
func (i *RegistriesConf) finish(source string) error {

	if string(i.File.Data) != defaultRegistriesConf {
		return i.validateReleaseImageIsSameInRegistriesConf(source)
	}
...
func (i *RegistriesConf) validateReleaseImageIsSameInRegistriesConf(source string) error {
	...
	if !strings.Contains(string(i.File.Data), releaseImagePath) {
		return errors.New(fmt.Sprintf("%s %s should have an entry matching the releaseImage %s", source, RegistriesConfFilename, releaseImagePath))
	}
