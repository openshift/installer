...
`,
			expectedFound: false,
			expectedError: "invalid install-config configuration: [controlPlane.fencing.credentials: Forbidden: there should be exactly two fencing credentials to support the two node cluster, instead 0 credentials were found, controlPlane.replicas: Unsupported value: 2: supported values: \"3\", \"1\", \"4\", \"5\", \"2 (with fencing)\"]",
		},
		{
...
`,
			expectedFound: false,
			expectedError: `invalid install-config configuration: [platform.none.fencingCredentials: Forbidden: this field is protected by the DualReplica feature gate which must be enabled through either the TechPreviewNoUpgrade or CustomNoUpgrade feature set, controlPlane.replicas: Unsupported value: 2: supported values: "3", "1", "4", "5", "2 (with arbiter)"]`,
		},
		{
...
`,
			expectedFound: false,
			expectedError: `invalid install-config configuration: controlPlane.fencing.credentials: Forbidden: there should be exactly two fencing credentials to support the two node cluster, instead 1 credentials were found`,
		},
		{
