...
func provisionSecurityGroups(ctx context.Context, manifests *[]client.Object, installConfig *installconfig.InstallConfig, clusterID *installconfig.ClusterID, mastersSchedulable bool) error {
	...
	var masterGroup, workerGroup *groups.SecGroup
	{
		...
		if err := attributestags.Add(networkClient, "security-groups", masterGroup.ID, "openshiftClusterID="+clusterID.InfraID).ExtractErr(); err != nil {
		...
		if err := attributestags.Add(networkClient, "security-groups", workerGroup.ID, "openshiftClusterID="+clusterID.InfraID).ExtractErr(); err != nil {
	...
	go func() {
		...
		for ipVersion, anyIP := range map[rules.RuleEtherType][]string{
			rules.EtherType4: []string{"0.0.0.0/0"},
