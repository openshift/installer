...
func tagVIPPorts(ctx context.Context, installConfig *installconfig.InstallConfig, clusterID *installconfig.ClusterID) error {
	...
	if controlPlanePort := installConfig.Config.Platform.OpenStack.ControlPlanePort; controlPlanePort != nil {
		for _, vips := range [][]string{installConfig.Config.OpenStack.APIVIPs, installConfig.Config.OpenStack.IngressVIPs} {
			...
			if len(vips) > 0 {
				...
				if port != nil {
					err := attributestags.Add(networkClient, "ports", port.ID, clusterID.InfraID+openstackdefaults.DualStackVIPsPortTag).ExtractErr()
					if err != nil {
						return err
					}
				}
