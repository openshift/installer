...
func replaceBootstrapIgnitionInTFVars(ctx context.Context, tfvarsFile *asset.File, installConfig *installconfig.InstallConfig, clusterID *installconfig.ClusterID) error {
	...
	bootstrapShim, err := buildBootstrapShim(ctx, installConfig.Config.Platform.OpenStack.Cloud, clusterID.InfraID, installConfig.Config.Proxy, []byte(bootstrapIgnition))
	if err != nil {
...
func replaceBootstrapIgnitionInManifests(ctx context.Context, manifests *[]client.Object, installConfig *installconfig.InstallConfig, clusterID *installconfig.ClusterID) error {
	const (
		boostrapIgnitionSecretValueKey  = "value"
		boostrapIgnitionSecretFormatKey = "format"
	)

	var (
		bootstrapIgnitionSecret      *corev1.Secret
		bootstrapIgnitionSecretIndex int
	)

	for i := range *manifests {
		if (*manifests)[i].GetObjectKind().GroupVersionKind().Kind == "Secret" && (*manifests)[i].GetName() == clusterID.InfraID+"-bootstrap" {
			var ok bool
			bootstrapIgnitionSecret, ok = (*manifests)[i].(*corev1.Secret)
			if ok {
				bootstrapIgnitionSecretIndex = i
				break
			}
		}
	}
	if bootstrapIgnitionSecret == nil {
		return fmt.Errorf("bootstrap Ignition not found")
	}

	if format := bootstrapIgnitionSecret.Data[boostrapIgnitionSecretFormatKey]; !bytes.Equal(format, []byte("ignition")) {
		return fmt.Errorf("bootstrap Ignition format not recognised: %q", format)
	}

	bootstrapIgnition, ok := bootstrapIgnitionSecret.Data[boostrapIgnitionSecretValueKey]
	if !ok {
		return fmt.Errorf("bootstrap Ignition data not found")
	}

	logrus.Debugf("Uploading Ignition to Glance")
	bootstrapShim, err := buildBootstrapShim(ctx, installConfig.Config.Platform.OpenStack.Cloud, clusterID.InfraID, installConfig.Config.Proxy, bootstrapIgnition)
	if err != nil {
		return fmt.Errorf("failed to build bootstrap's Ignition shim: %w", err)
	}

	logrus.Debugf("Replacing the Ignition file in the CAPI manifests")
	bootstrapIgnitionSecret.Data[boostrapIgnitionSecretValueKey] = bootstrapShim
	(*manifests)[bootstrapIgnitionSecretIndex] = bootstrapIgnitionSecret
	return nil
}
...
func buildBootstrapShim(ctx context.Context, cloud string, infraID string, proxy *types.Proxy, bootstrapIgn []byte) ([]byte, error) {
