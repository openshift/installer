...
func (gateway *NatGateway_Spec) ConvertToARM(resolved genruntime.ConvertToARMResolvedDetails) (interface{}, error) {
	...
	if gateway.IdleTimeoutInMinutes != nil ||
		...
		gateway.PublicIpAddressesV6 != nil ||
		gateway.PublicIpPrefixes != nil ||
		gateway.PublicIpPrefixesV6 != nil ||
		gateway.SourceVirtualNetwork != nil {
	...
	for _, item := range gateway.PublicIpAddressesV6 {
		item_ARM, err := item.ConvertToARM(resolved)
		if err != nil {
			return nil, err
		}
		result.Properties.PublicIpAddressesV6 = append(result.Properties.PublicIpAddressesV6, *item_ARM.(*arm.SubResource))
	}
	...
	for _, item := range gateway.PublicIpPrefixesV6 {
		item_ARM, err := item.ConvertToARM(resolved)
		if err != nil {
			return nil, err
		}
		result.Properties.PublicIpPrefixesV6 = append(result.Properties.PublicIpPrefixesV6, *item_ARM.(*arm.SubResource))
	}
	if gateway.SourceVirtualNetwork != nil {
		sourceVirtualNetwork_ARM, err := gateway.SourceVirtualNetwork.ConvertToARM(resolved)
		if err != nil {
			return nil, err
		}
		sourceVirtualNetwork := *sourceVirtualNetwork_ARM.(*arm.SubResource)
		result.Properties.SourceVirtualNetwork = &sourceVirtualNetwork
	}
...
func (gateway *NatGateway_Spec) PopulateFromARM(owner genruntime.ArbitraryOwnerReference, armInput interface{}) error {
	...
	// Set property "PublicIpAddressesV6":
	// copying flattened property:
	if typedInput.Properties != nil {
		for _, item := range typedInput.Properties.PublicIpAddressesV6 {
			var item1 SubResource
			err := item1.PopulateFromARM(owner, item)
			if err != nil {
				return err
			}
			gateway.PublicIpAddressesV6 = append(gateway.PublicIpAddressesV6, item1)
		}
	}
	...
	// Set property "PublicIpPrefixesV6":
	// copying flattened property:
	if typedInput.Properties != nil {
		for _, item := range typedInput.Properties.PublicIpPrefixesV6 {
			var item1 SubResource
			err := item1.PopulateFromARM(owner, item)
			if err != nil {
				return err
			}
			gateway.PublicIpPrefixesV6 = append(gateway.PublicIpPrefixesV6, item1)
		}
	}
	...
	// Set property "SourceVirtualNetwork":
	// copying flattened property:
	if typedInput.Properties != nil {
		if typedInput.Properties.SourceVirtualNetwork != nil {
			var sourceVirtualNetwork1 SubResource
			err := sourceVirtualNetwork1.PopulateFromARM(owner, *typedInput.Properties.SourceVirtualNetwork)
			if err != nil {
				return err
			}
			sourceVirtualNetwork := sourceVirtualNetwork1
			gateway.SourceVirtualNetwork = &sourceVirtualNetwork
		}
	}
...
func (gateway *NatGateway_Spec) AssignProperties_From_NatGateway_Spec(source *storage.NatGateway_Spec) error {
	...
	// PublicIpAddressesV6
	if source.PublicIpAddressesV6 != nil {
		publicIpAddressesV6List := make([]SubResource, len(source.PublicIpAddressesV6))
		for publicIpAddressesV6Index, publicIpAddressesV6Item := range source.PublicIpAddressesV6 {
			var publicIpAddressesV6 SubResource
			err := publicIpAddressesV6.AssignProperties_From_SubResource(&publicIpAddressesV6Item)
			if err != nil {
				return eris.Wrap(err, "calling AssignProperties_From_SubResource() to populate field PublicIpAddressesV6")
			}
			publicIpAddressesV6List[publicIpAddressesV6Index] = publicIpAddressesV6
		}
		gateway.PublicIpAddressesV6 = publicIpAddressesV6List
	} else {
		gateway.PublicIpAddressesV6 = nil
	}
	...
	// PublicIpPrefixesV6
	if source.PublicIpPrefixesV6 != nil {
		publicIpPrefixesV6List := make([]SubResource, len(source.PublicIpPrefixesV6))
		for publicIpPrefixesV6Index, publicIpPrefixesV6Item := range source.PublicIpPrefixesV6 {
			var publicIpPrefixesV6 SubResource
			err := publicIpPrefixesV6.AssignProperties_From_SubResource(&publicIpPrefixesV6Item)
			if err != nil {
				return eris.Wrap(err, "calling AssignProperties_From_SubResource() to populate field PublicIpPrefixesV6")
			}
			publicIpPrefixesV6List[publicIpPrefixesV6Index] = publicIpPrefixesV6
		}
		gateway.PublicIpPrefixesV6 = publicIpPrefixesV6List
	} else {
		gateway.PublicIpPrefixesV6 = nil
	}
	...
	if source.SourceVirtualNetwork != nil {
		var sourceVirtualNetwork SubResource
		err := sourceVirtualNetwork.AssignProperties_From_SubResource(source.SourceVirtualNetwork)
		if err != nil {
			return eris.Wrap(err, "calling AssignProperties_From_SubResource() to populate field SourceVirtualNetwork")
		}
		gateway.SourceVirtualNetwork = &sourceVirtualNetwork
	} else {
		gateway.SourceVirtualNetwork = nil
...
func (gateway *NatGateway_Spec) AssignProperties_To_NatGateway_Spec(destination *storage.NatGateway_Spec) error {
	...
	// PublicIpAddressesV6
	if gateway.PublicIpAddressesV6 != nil {
		publicIpAddressesV6List := make([]storage.SubResource, len(gateway.PublicIpAddressesV6))
		for publicIpAddressesV6Index, publicIpAddressesV6Item := range gateway.PublicIpAddressesV6 {
			var publicIpAddressesV6 storage.SubResource
			err := publicIpAddressesV6Item.AssignProperties_To_SubResource(&publicIpAddressesV6)
			if err != nil {
				return eris.Wrap(err, "calling AssignProperties_To_SubResource() to populate field PublicIpAddressesV6")
			}
			publicIpAddressesV6List[publicIpAddressesV6Index] = publicIpAddressesV6
		}
		destination.PublicIpAddressesV6 = publicIpAddressesV6List
	} else {
	...
	// PublicIpPrefixesV6
	if gateway.PublicIpPrefixesV6 != nil {
		publicIpPrefixesV6List := make([]storage.SubResource, len(gateway.PublicIpPrefixesV6))
		for publicIpPrefixesV6Index, publicIpPrefixesV6Item := range gateway.PublicIpPrefixesV6 {
			var publicIpPrefixesV6 storage.SubResource
			err := publicIpPrefixesV6Item.AssignProperties_To_SubResource(&publicIpPrefixesV6)
			if err != nil {
				return eris.Wrap(err, "calling AssignProperties_To_SubResource() to populate field PublicIpPrefixesV6")
			}
			publicIpPrefixesV6List[publicIpPrefixesV6Index] = publicIpPrefixesV6
		}
		destination.PublicIpPrefixesV6 = publicIpPrefixesV6List
	} else {
	...
	// SourceVirtualNetwork
	if gateway.SourceVirtualNetwork != nil {
		var sourceVirtualNetwork storage.SubResource
		err := gateway.SourceVirtualNetwork.AssignProperties_To_SubResource(&sourceVirtualNetwork)
		if err != nil {
			return eris.Wrap(err, "calling AssignProperties_To_SubResource() to populate field SourceVirtualNetwork")
		}
...
func (gateway *NatGateway_Spec) Initialize_From_NatGateway_STATUS(source *NatGateway_STATUS) error {
	...
	if source.PublicIpAddressesV6 != nil {
		...
		for publicIpAddressesV6Index, publicIpAddressesV6Item := range source.PublicIpAddressesV6 {
			var publicIpAddressesV6 SubResource
			err := publicIpAddressesV6.Initialize_From_SubResource_STATUS(&publicIpAddressesV6Item)
			if err != nil {
				return eris.Wrap(err, "calling Initialize_From_SubResource_STATUS() to populate field PublicIpAddressesV6")
			}
			publicIpAddressesV6List[publicIpAddressesV6Index] = publicIpAddressesV6
		}
		gateway.PublicIpAddressesV6 = publicIpAddressesV6List
	...
	if source.PublicIpPrefixesV6 != nil {
		...
		for publicIpPrefixesV6Index, publicIpPrefixesV6Item := range source.PublicIpPrefixesV6 {
			var publicIpPrefixesV6 SubResource
			err := publicIpPrefixesV6.Initialize_From_SubResource_STATUS(&publicIpPrefixesV6Item)
			if err != nil {
				return eris.Wrap(err, "calling Initialize_From_SubResource_STATUS() to populate field PublicIpPrefixesV6")
			}
			publicIpPrefixesV6List[publicIpPrefixesV6Index] = publicIpPrefixesV6
		}
		gateway.PublicIpPrefixesV6 = publicIpPrefixesV6List
	...
	if source.SourceVirtualNetwork != nil {
		var sourceVirtualNetwork SubResource
		err := sourceVirtualNetwork.Initialize_From_SubResource_STATUS(source.SourceVirtualNetwork)
		if err != nil {
			return eris.Wrap(err, "calling Initialize_From_SubResource_STATUS() to populate field SourceVirtualNetwork")
		}
		gateway.SourceVirtualNetwork = &sourceVirtualNetwork
	} else {
		gateway.SourceVirtualNetwork = nil
...
type NatGateway_STATUS struct {
	...
	// PublicIpAddressesV6: An array of public ip addresses V6 associated with the nat gateway resource.
	PublicIpAddressesV6 []SubResource_STATUS `json:"publicIpAddressesV6,omitempty"`

	// PublicIpPrefixes: An array of public ip prefixes V4 associated with the nat gateway resource.
	PublicIpPrefixes []SubResource_STATUS `json:"publicIpPrefixes,omitempty"`

	// PublicIpPrefixesV6: An array of public ip prefixes V6 associated with the nat gateway resource.
	PublicIpPrefixesV6 []SubResource_STATUS `json:"publicIpPrefixesV6,omitempty"`
	...
	// SourceVirtualNetwork: A reference to the source virtual network using this nat gateway resource.
	SourceVirtualNetwork *SubResource_STATUS `json:"sourceVirtualNetwork,omitempty"`
...
func (gateway *NatGateway_STATUS) PopulateFromARM(owner genruntime.ArbitraryOwnerReference, armInput interface{}) error {
	...
	if typedInput.Properties != nil {
		if typedInput.Properties.SourceVirtualNetwork != nil {
			var sourceVirtualNetwork1 SubResource_STATUS
			err := sourceVirtualNetwork1.PopulateFromARM(owner, *typedInput.Properties.SourceVirtualNetwork)
			if err != nil {
				return err
			}
...
func (gateway *NatGateway_STATUS) AssignProperties_From_NatGateway_STATUS(source *storage.NatGateway_STATUS) error {
	...
	// PublicIpAddressesV6
	if source.PublicIpAddressesV6 != nil {
		publicIpAddressesV6List := make([]SubResource_STATUS, len(source.PublicIpAddressesV6))
		for publicIpAddressesV6Index, publicIpAddressesV6Item := range source.PublicIpAddressesV6 {
			var publicIpAddressesV6 SubResource_STATUS
			err := publicIpAddressesV6.AssignProperties_From_SubResource_STATUS(&publicIpAddressesV6Item)
			if err != nil {
				return eris.Wrap(err, "calling AssignProperties_From_SubResource_STATUS() to populate field PublicIpAddressesV6")
			}
			publicIpAddressesV6List[publicIpAddressesV6Index] = publicIpAddressesV6
		}
		gateway.PublicIpAddressesV6 = publicIpAddressesV6List
	...
	// PublicIpPrefixesV6
	if source.PublicIpPrefixesV6 != nil {
		publicIpPrefixesV6List := make([]SubResource_STATUS, len(source.PublicIpPrefixesV6))
		for publicIpPrefixesV6Index, publicIpPrefixesV6Item := range source.PublicIpPrefixesV6 {
			var publicIpPrefixesV6 SubResource_STATUS
			err := publicIpPrefixesV6.AssignProperties_From_SubResource_STATUS(&publicIpPrefixesV6Item)
			if err != nil {
				return eris.Wrap(err, "calling AssignProperties_From_SubResource_STATUS() to populate field PublicIpPrefixesV6")
			}
			publicIpPrefixesV6List[publicIpPrefixesV6Index] = publicIpPrefixesV6
		}
		gateway.PublicIpPrefixesV6 = publicIpPrefixesV6List
	...
	// SourceVirtualNetwork
	if source.SourceVirtualNetwork != nil {
		var sourceVirtualNetwork SubResource_STATUS
		err := sourceVirtualNetwork.AssignProperties_From_SubResource_STATUS(source.SourceVirtualNetwork)
		if err != nil {
			return eris.Wrap(err, "calling AssignProperties_From_SubResource_STATUS() to populate field SourceVirtualNetwork")
		}
		gateway.SourceVirtualNetwork = &sourceVirtualNetwork
	} else {
		gateway.SourceVirtualNetwork = nil
...
func (gateway *NatGateway_STATUS) AssignProperties_To_NatGateway_STATUS(destination *storage.NatGateway_STATUS) error {
	...
	// PublicIpAddressesV6
	if gateway.PublicIpAddressesV6 != nil {
		publicIpAddressesV6List := make([]storage.SubResource_STATUS, len(gateway.PublicIpAddressesV6))
		for publicIpAddressesV6Index, publicIpAddressesV6Item := range gateway.PublicIpAddressesV6 {
			var publicIpAddressesV6 storage.SubResource_STATUS
			err := publicIpAddressesV6Item.AssignProperties_To_SubResource_STATUS(&publicIpAddressesV6)
			if err != nil {
				return eris.Wrap(err, "calling AssignProperties_To_SubResource_STATUS() to populate field PublicIpAddressesV6")
			}
			publicIpAddressesV6List[publicIpAddressesV6Index] = publicIpAddressesV6
		}
		destination.PublicIpAddressesV6 = publicIpAddressesV6List
	...
	// PublicIpPrefixesV6
	if gateway.PublicIpPrefixesV6 != nil {
		publicIpPrefixesV6List := make([]storage.SubResource_STATUS, len(gateway.PublicIpPrefixesV6))
		for publicIpPrefixesV6Index, publicIpPrefixesV6Item := range gateway.PublicIpPrefixesV6 {
			var publicIpPrefixesV6 storage.SubResource_STATUS
			err := publicIpPrefixesV6Item.AssignProperties_To_SubResource_STATUS(&publicIpPrefixesV6)
			if err != nil {
				return eris.Wrap(err, "calling AssignProperties_To_SubResource_STATUS() to populate field PublicIpPrefixesV6")
			}
			publicIpPrefixesV6List[publicIpPrefixesV6Index] = publicIpPrefixesV6
		}
		destination.PublicIpPrefixesV6 = publicIpPrefixesV6List
	...
	// SourceVirtualNetwork
	if gateway.SourceVirtualNetwork != nil {
		var sourceVirtualNetwork storage.SubResource_STATUS
		err := gateway.SourceVirtualNetwork.AssignProperties_To_SubResource_STATUS(&sourceVirtualNetwork)
		if err != nil {
			return eris.Wrap(err, "calling AssignProperties_To_SubResource_STATUS() to populate field SourceVirtualNetwork")
		}
		destination.SourceVirtualNetwork = &sourceVirtualNetwork
	} else {
		destination.SourceVirtualNetwork = nil
...
const (
	NatGatewaySku_Name_STATUS_Standard   = NatGatewaySku_Name_STATUS("Standard")
	NatGatewaySku_Name_STATUS_StandardV2 = NatGatewaySku_Name_STATUS("StandardV2")
)
...
var natGatewaySku_Name_STATUS_Values = map[string]NatGatewaySku_Name_STATUS{
	...
	"standardv2": NatGatewaySku_Name_STATUS_StandardV2,
}
