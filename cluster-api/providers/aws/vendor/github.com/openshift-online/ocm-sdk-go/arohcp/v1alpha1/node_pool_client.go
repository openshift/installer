...
// Status returns the target 'node_pool_status' resource.
func (c *NodePoolClient) Status() *NodePoolStatusClient {
	return NewNodePoolStatusClient(
		c.transport,
...
// NodePoolDeleteRequest is the request for the 'async_delete' method.
...
// NodePoolDeleteResponse is the response for the 'async_delete' method.
...
// NodePoolUpdateRequest is the request for the 'async_update' method.
...
func (r *NodePoolUpdateRequest) SendContext(ctx context.Context) (result *NodePoolUpdateResponse, err error) {
	...
	header := helpers.CopyHeader(r.header)
	buffer := &bytes.Buffer{}
	err = writeNodePoolAsyncUpdateRequest(r, buffer)
	if err != nil {
		return
	}
	uri := &url.URL{
	...
	err = readNodePoolAsyncUpdateResponse(result, reader)
	if err != nil {
		return
	}
	return
...
// NodePoolUpdateResponse is the response for the 'async_update' method.
