...
// AWSClusterStaticIdentity implements a validating and defaulting webhook for AWSClusterStaticIdentity.
...
func (w *AWSClusterStaticIdentity) SetupWebhookWithManager(mgr ctrl.Manager) error {
	return ctrl.NewWebhookManagedBy(mgr).
		For(&infrav1.AWSClusterStaticIdentity{}).
		WithValidator(w).
		WithDefaulter(w).
		Complete()
}
...
func (*AWSClusterStaticIdentity) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	r, ok := obj.(*infrav1.AWSClusterStaticIdentity)
	if !ok {
...
func (*AWSClusterStaticIdentity) ValidateDelete(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	return nil, nil
}

// ValidateUpdate implements webhook.CustomValidator so a webhook will be registered for the type.
func (*AWSClusterStaticIdentity) ValidateUpdate(_ context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	r, ok := newObj.(*infrav1.AWSClusterStaticIdentity)
	if !ok {
		return nil, fmt.Errorf("expected an AWSClusterStaticIdentity object but got %T", r)
	}

	oldP, ok := oldObj.(*infrav1.AWSClusterStaticIdentity)
	if !ok {
...
func (*AWSClusterStaticIdentity) Default(_ context.Context, obj runtime.Object) error {
	r, ok := obj.(*infrav1.AWSClusterStaticIdentity)
	if !ok {
