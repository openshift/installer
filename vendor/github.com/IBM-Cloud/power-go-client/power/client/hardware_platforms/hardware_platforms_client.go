...
// New creates a new hardware platforms API client with basic auth credentials.
...
// New creates a new hardware platforms API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) ServiceBrokerHardwareplatformsGet(params *ServiceBrokerHardwareplatformsGetParams, opts ...ClientOption) (*ServiceBrokerHardwareplatformsGetOK, error) {
	...
	success, ok := result.(*ServiceBrokerHardwareplatformsGetOK)
	if ok {
		return success, nil
	}
	...
	msg := fmt.Sprintf("unexpected success response for serviceBroker.hardwareplatforms.get: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
