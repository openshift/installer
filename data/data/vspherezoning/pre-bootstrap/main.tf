...
/* This might end up being documentation for the 4.12
 * release. Leaving here temporarily.

resource "vsphere_tag_category" "region_category" {
  name = local.region_tag_category 
  description = "Added by openshift-install do not remove"
  cardinality = "SINGLE"
...
resource "vsphere_tag" "region_tag" {
  for_each = var.vsphere_deployment_zone
  name = var.vsphere_failure_zone[each.value.failureDomain].region.name

  category_id = vsphere_tag_category.region_category.id
  description = "Added by openshift-install do not remove"
}

resource "vsphere_tag_category" "zone_category" {

  name = local.zone_tag_category 
...
resource "vsphere_tag" "zone_tag" {
  for_each = var.vsphere_deployment_zone
  name = var.vsphere_failure_zone[each.value.failureDomain].zone.name
  category_id = vsphere_tag_category.zone_category.id
  description = "Added by openshift-install do not remove"
}

resource "vsphereprivate_tag_attach" "regions" {
  for_each = var.vsphere_deployment_zone
  objectid   = data.vsphere_datacenter.datacenter[each.key].id
  objecttype = "Datacenter"

  tagid = vsphere_tag.region_tag[each.key].id
}

resource "vsphereprivate_tag_attach" "zones" {
  for_each = var.vsphere_deployment_zone
  objectid   = data.vsphere_compute_cluster.cluster[each.key].id
  objecttype = "ClusterComputeResource"

  tagid = vsphere_tag.zone_tag[each.key].id
}
*/
