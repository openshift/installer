data "aws_route_table" "private_routes" {
...
data "aws_route" "to_nat_gw" {
  count = var.private_subnets == null ? length(var.availability_zones) : 0

  route_table_id         = data.aws_route_table.private_routes[count.index].id
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = element(data.aws_nat_gateway.nat_gw.*.id, count.index)
}
