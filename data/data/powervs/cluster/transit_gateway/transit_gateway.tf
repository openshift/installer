...
resource "ibm_tg_gateway" "transit_gateway" {
  count          = var.attached_transit_gateway == "" && var.tg_name == "" ? 1 : 0
...
data "ibm_tg_gateway" "existing_transit_gateway" {
  count = var.tg_name != "" ? 1 : 0
  name  = var.tg_name
}
...
resource "ibm_tg_connection" "tg_connection_vpc" {
  ...
  gateway      = var.attached_transit_gateway == "" ? (var.tg_name == "" ? resource.ibm_tg_gateway.transit_gateway[0].id : data.ibm_tg_gateway.existing_transit_gateway[0].id) : var.attached_transit_gateway
  network_type = "vpc"
...
resource "ibm_tg_connection" "tg_connection_pvs" {
  ...
  gateway      = var.tg_name == "" ? resource.ibm_tg_gateway.transit_gateway[count.index].id : data.ibm_tg_gateway.existing_transit_gateway[0].id
