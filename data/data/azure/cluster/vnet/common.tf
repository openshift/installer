...
locals {
  ...
  master_subnet_id = var.preexisting_network ? data.azurerm_subnet.preexisting_master_subnet[0].id : data.azurerm_subnet.master_subnet[0].id
  worker_subnet_id = var.preexisting_network ? data.azurerm_subnet.preexisting_worker_subnet[0].id : data.azurerm_subnet.worker_subnet[0].id

  virtual_network    = var.preexisting_network ? data.azurerm_virtual_network.preexisting_virtual_network[0].name : data.azurerm_virtual_network.cluster_vnet[0].name
  virtual_network_id = var.preexisting_network ? data.azurerm_virtual_network.preexisting_virtual_network[0].id : data.azurerm_virtual_network.cluster_vnet[0].id
