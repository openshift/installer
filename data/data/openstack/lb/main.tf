...
data "ignition_file" "openshift_hosts" {
  ...
  content {
    content = <<EOF
${replace(join("\n", formatlist("%s ${var.cluster_name}-etcd-%s.${var.cluster_domain}", var.controlplane_ips, var.controlplane_port_names)), "controlplane-port-", "")}
EOF
  }
}
