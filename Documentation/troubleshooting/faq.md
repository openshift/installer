...
For example, if the Kubernetes version is 1.7.1, the first Tectonic production release including it is labeled `1.7.1-tectonic.1`. A second release including the same 1.7.1 version of Kubernetes would be `1.7.1-tectonic.2`. When the version number of the included Kubernetes advances to 1.7.2, the associated Tectonic version number would be `1.7.2-tectonic.1`.
